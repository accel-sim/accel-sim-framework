...
void printHelp() {
  ...
  printf("-n <number of samples>: \tNumber of power samples to collect (-1 == "
         "poll until CTRL+C)\n");
  printf("-p: \t\t\t\tPrint PSU info as well \n");
  printf("-a: <process name>: \t\tApplication to profile with this profiler. "
         "Profiler will stop when process terminates\n");
  printf("-t: <temp>:\t\tCutoff temperature in degrees C. Profiler will stop "
...
int main(int argc, char **argv) {
  ...
  if (pname != NULL) {
    ...
    if (fgets(line, sizeof(line), f)) {
      ...
      pclose(f);
      printf("Error: application not found in process list. Make sure to start "
