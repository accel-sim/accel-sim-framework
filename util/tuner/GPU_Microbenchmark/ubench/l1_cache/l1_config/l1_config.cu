...
// we know sector size that from l1_assoc and l1_acces_grain ubenches and has
...
int main() {
  ...
  if (ACCEL_SIM_MODE) {
    ...
    // l1_mshr ubench)
    unsigned mshr = warps_num_per_sm * access_factor * 2;
    ...
    // allocate yet
    std::cout << "-gpgpu_cache:dl1 S:" << L1_CACHE_SETS << ","
